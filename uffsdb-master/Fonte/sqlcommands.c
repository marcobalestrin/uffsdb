...
inf_where *novoResWhere(void *tk, int id) {
    inf_where *novo = malloc(sizeof(inf_where));
    if (novo == NULL) {
        fprintf(stderr, "Erro ao alocar memória para novo inf_where.\n");
        return NULL;
    }
    ...
    if (id == STRING || id == ABRE_PARENT || id == FECHA_PARENT) {
        novo->token = novoTokenWhere((char *)tk, id);
    } else if (id == VALUE_NUMBER) {
        double *tok = malloc(sizeof(double));
        if (tok == NULL) {
            fprintf(stderr, "Erro ao alocar memória para token double.\n");
            free(novo);
            return NULL;
        }
        *tok = *((double *)tk);
        ...
        if (tok == NULL) {
            fprintf(stderr, "Erro ao alocar memória para token char.\n");
            free(novo);
            return NULL;
        }
        *tok = *((char *)tk);
...
void printConsulta(Lista *p,Lista *l){
  ...
  printf("\n %d Linha%s.\n",l->tam,l->tam == 1 ? "" : "s");
  p=NULL; //reinicializa o SELECT.proj(p) para um futura consulta
}
